...
class Panel extends BaseController
{
    ...
    {
        $session = session();
        // kullanıcı giriş yapmamışsa logine yönlendirilir
    ...
    public function kayit_ekle()
    {
        ...
        if($session->has('durum') && $session->get('durum'))
        {
            ...
            //validator veri gelir doğrulanır ve yazdırılır

            $veri = $this->validator->getValidated();
            $model = model('PanelModel');

            $img = $this->request->getFile('resim');
            //HTTP POST isteği ile gönderilen dosyayı almak için kullanılır.
